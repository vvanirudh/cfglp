...
int Register_Descriptor::get_size_of_list()
{
...
void Lra_Outcome::optimize_lra(Lra_Scenario lcase, Ast * destination_memory, Ast * source_memory)
{
	...
	case mc_2m:
		...
		{
			if(destination_register->get_size_of_list()==1){
	...
	case mc_2r:
		...
		if(typeid(*source_memory)==typeid(Name_Ast)){
			source_symbol_entry = &(source_memory->get_symbol_entry());
			source_register = source_symbol_entry->get_register(); 
			destination_symbol_entry = &(source_memory->get_symbol_entry()); //MY ADDITION
		}
		else
		{
		...
		else 
		{
			...
			Symbol_Table_Entry & sym = *new Symbol_Table_Entry();
			result_register->update_symbol_information(sym);
		}
